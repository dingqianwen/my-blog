<template>
  <div>
    ...
    <div class="music_player" :style="(!isLargeScreen) ?' display: block':'display: flex'">

      <div class="left" v-show="!isLoading && (isLargeScreen || switchPanel)"
           :style="isLargeScreen?'padding: 80px 120px 0 15px;':'padding: 80px 0px 0 0px;margin: 0 auto;align-items: center;'">
      ...
      <!--适配移动端，控制器从左侧移动到下面显示-->
      <div class="pcl" v-show="!isLoading && (!isLargeScreen)"
           style="padding: 10px 0 0 0; margin: 0 auto;">
        <div class="progress-bar-wrap">
          <div class="time">{{ formatTime(currentTime) }} / {{ formatTime(totalTime) }}</div>
          <div @click="(e)=>progressClick(e)"
               style="cursor: pointer;">
